...
if __name__ == '__main__':
    ...
    for experiment in range(NUM_EXPERIMENTS):
        ...
        evaluation_data_all = [i[0:-1] for i in dataset]
        ...
        # Procesar las secuencias hasta llegar a los 6 prototipos iniciales
        for id_object, sequence in enumerate(training_data):
            #print(sequence)
            predicted_label = nearest_mean_classifier.predict(sequence, id_object, str(id_object)+'_0')
            if predicted_label == -1:
                print("Creados todos los prototipos iniciales...")
                break
            elif predicted_label >= 0:
                index_correspondence[str(predicted_label)] = id_object
                csv_data[str(predicted_label)] = [str(id_object)+'_0']
        
        # Descartamos los datos utilizados para inicializar el modelo
        #evaluation_data = [sample for i, sample in enumerate(evaluation_data_all) if i >= len(nearest_mean_classifier.prototypes)]
    ...
    df_f1_transpuesto.to_csv('resultados_f1_score_ncm.csv', index=False, header=False)
    df_overlap_transpuesto.to_csv('resultados_overlap_ncm.csv', index=False, header=False) 
