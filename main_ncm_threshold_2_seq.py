...
EXPERIMENT_NAME='ncm_threshold_2_seq'
NUM_EXPERIMENTS = 10
num_run=0
...
if __name__ == '__main__':
    ...
    for i in range(2, 20, 1):
        ...
        for experiment in range(NUM_EXPERIMENTS):
            seed = int(str(i) + str(experiment+1))
            ...
            while individuos:
                ...
                print("Individuo elegido: ", individuo)
                print("Secuencias restantes: ", len(training_data[individuo]))
                # Obtenemos la primera y segunda secuencia (si existe) del individuo
                if(len(training_data[individuo]) > 1):
                    sequences.append(training_data[individuo][0])
                    sequences.append(training_data[individuo][1])
                else:
                    sequences.append(training_data[individuo][0])
                ...
                for sequence in sequences:
                    ...
                    etiqueta = str(user_counter)+'_'+str(final_etiqueta)  # Ejemplo: 0_0

                    predicted_label = nearest_mean_classifier.predict(sequence, user_counter, etiqueta)
                    ...
                    if predicted_label == -1:
                        ...
                        user_counter += 1
                        index_correspondence[str(predicted_label)] = user_counter
                        csv_data[str(predicted_label)] = [etiqueta]
                    elif predicted_label >= 0:
                        csv_data[str(predicted_label)].append(str(individuo)+'_'+str(final_etiqueta+1))
    ...
    df_f1_pivot.to_csv('resultados_f1_score_ncm_threshold_2_seq.csv', index=False)
    df_overlap_pivot.to_csv('resultados_overlap_ncm_threshold_2_seq.csv', index=False) 
