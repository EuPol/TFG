...
class Gaussian_Mixture_Model(object):
    ...
    def inv_cov(self, covariance):
        ...
        is_det_zero = torch.isclose(det, torch.zeros(1, dtype=torch.double).to(self.device), atol=1e-04)
        if is_det_zero:
            ...
            if m == 0:
                ...
                for j in range(inv_covariance.shape[0]):
                    if torch.isclose(inv_covariance[j, j], torch.zeros(1, dtype=torch.double).to(self.device), atol=1e-05):
    ...
    def predict(self, FV):
        ...
        if self.number_of_classes == 0:
            ...
            q = torch.zeros(FV.shape[0], self.number_of_classes).to(self.device)
            for j, (k, mu) in enumerate(self.center_dict.items()):
                if mu != None:
                    mu=mu.to(self.device)
                    inv_covariance_j = self.inverse_covariance_dict[k].to(self.device)
            ...
            u = 1 - m
            probability_tensor = torch.zeros(FV.shape[0], 1 + self.number_of_classes).to(self.device)
