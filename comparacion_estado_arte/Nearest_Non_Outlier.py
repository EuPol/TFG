...
class Nearest_classifier_Mean_Metric_learning(torch.nn.Module):
    def __init__(self, centers, rank=2, W=None):
        ...
        self.W = torch.nn.Linear(feature_size, rank, bias=False).double()
        if W != None:
...
class Nearest_Non_Outlier(object):
    def __init__(
        ...
        if gpu != None: #Modified
            self.SoftMax = torch.nn.Softmax(dim=1).to(gpu)
            self.loss_CE = torch.nn.CrossEntropyLoss(reduction="mean").to(gpu)
        else:
    ...
    def fit(self, class_to_process, features_dict_new):
        ...
        if self.gpu != None:
            self.centers = self.centers.to(self.gpu)
        ...
        for epoch in range(self.max_number_of_epoch):
            ...
            for i, (x, y) in enumerate(train_loader, 0):
                optimizer.zero_grad()
                if self.gpu != None:
                    x = x.double().to(self.gpu)
                    y = y.long().to(self.gpu)
                else:
                    x=x.double()
                    y = y.long()
    ...
    def predict(self, FV):
        ...
        if self.number_of_classes == 0:
            ...
            with torch.no_grad():
                ...
                elementwise_known = (f_hat > 0).double()#float()
                ...
                if self.gpu != None:
                    probability_tensor = torch.zeros(FV.shape[0], 1 + self.number_of_classes).to(self.gpu).double()
                else:
                    probability_tensor = torch.zeros(FV.shape[0], 1 + self.number_of_classes).double()
