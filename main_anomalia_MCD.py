...
import pandas as pd
import os
from anomalia_MCD import AnomalyClass
...
EXPERIMENT_NAME='anomaly_MCD_seq'
NUM_EXPERIMENTS = 10
num_run=0
num_frames=4
        ...
        for index_individuo, individuo in enumerate(training_data):
            for index_secuencia, secuencia in enumerate(individuo):
                ...
                if not anomaly_classes:
                    new_class = AnomalyClass("Class_1", user_counter)  # Asigna una etiqueta de clase inicial y un ID único
                    ...
                    if any(resultados):  # Si se ajusta a alguna clase existente
                        ...
                        new_class = AnomalyClass(new_class_label, user_counter)
